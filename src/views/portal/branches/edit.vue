<template>
    ...
    >
        <div
            :class="branchForm.isActive ? '' : 'opacity-60'"
            class="inner-container shadow-[0px_2px_8px_0px_rgba(10,102,194,0.07)] max-[699px]:!w-full h-[100%] flex flex-col bg-[#FAFAFA] min-[700px]:flex-row min-[700px]:h-[70%] min-[700px]:rounded-3xl min-[700px]:w-[85%] min-[1024px]:w-[75%] min-[1280px]:w-[70%] min-[1280px]:h-[75%] max-[1150px]:!w-[80%]"
        >
            <div
                class="right-box z-50 shadow-[0px_2px_8px_0px_rgba(10,102,194,0.07)] flex flex-col justify-center items-center bg-white max-[700px]:w-full max-[700px]:h-[40%] min-[700px]:w-[35%] max-[1200px]:w-[38%] min-[700px]:h-full min-[700px]:rounded-s-3xl min-[1024px]:w-[30%]"
            >
                <div class="flex flex-col gap-2 max-[700px]:gap-1 items-center w-[50%]">
                    <div class="flex relative justify-center items-center">
                        <div
                            class="flex justify-center w-32 h-32 border border-gray-100 items-center rounded-full max-[700px]:w-20 max-[700px]:h-20 max-[1280px]:w-24 max-[1280px]:h-24"
                        >
                            <img
                                class="company-profile object-cover h-full w-full rounded-full"
                                :src="branchForm?.company?.companyLogo || defaultProfile"
                                alt=""
                    ...
                    <div class="flex flex-col min-[700px]:gap-6 max-[700px]:gap-2 items-center justify-center">
                        <div class="flex flex-col gap-2 items-center text-center max-[700px]:gap-1">
                            <button
                                v-tippy:click
                        ...
                        <div class="flex flex-col gap-2 text-[#2375C8] font-semibold">
                            <div class="flex gap-2 items-center">
                                <button
                                    @click="submit_branch_form"
                                    type="submit"
                                    class="border border-[#2375C8] rounded px-6 py-1.5 flex gap-1 items-center max-[1150px]:px-3 max-[1150px]:text-sm max-[700px]:py-1"
                                >
                                    ذخیره
                                </button>
                                <button
                                    @click="cancel"
                                    type="button"
                                    class="border border-[#2375C8] rounded px-5 py-1.5 flex gap-1 items-center max-[1150px]:px-3 max-[1150px]:text-sm max-[700px]:py-1"
                                >
                                    انصراف
                                </button>
                            </div>
                            <button
                                ...
                                :class="branchForm.isActive ? 'bg-red-400' : 'bg-green-600'"
                                type="button"
                                class="border text-white text-center rounded px-5 py-1.5 items-center max-[1150px]:px-3 max-[1150px]:text-sm max-[700px]:py-1"
                            >
                                {{ branchForm.isActive ? 'غیرفعال کردن شعبه' : 'پرداخت و فعال سازی شعبه' }}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div
                class="left-box flex flex-col justify-center items-center w-[77%] max-[700px]:w-full max-[700px]:h-[60%] min-[700px]:w-[70%] min-[700px]:h-[100%] min-[1024px]:w-[75%]"
            >
                <button
                    @click="go_back"
                    class="h-[10%] rounded-tl-3xl bg-white shadow-[0px_2px_8px_0px_rgba(10,102,194,0.07)] w-full flex items-center justify-end pe-7 max-[700px]:rounded-none"
                >
                    <svg class="max-[800px]:w-5 max-[800px]:h-5" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M20 12H4M4 12L10 6M4 12L10 18" stroke="#757575" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </button>
                <form class="flex justify-center w-[85%] h-[90%] flex-col space-x-10 max-[700px]:space-y-4" @submit.prevent="submit_branch_form()">
                    <div
                        class="grid h-[90%] py-3 overflow-y-auto overflow-x-hidden grid-cols-2 gap-x-5 max-[699px]:grid-cols-1 max-[699px]:gap-y-3 min-[700px]:py-0"
                    >
                        <div>
                        ...
                        <div>
                            ...
                            >
                            <div class="relative z-40 bg-white h-[38px] max-[1500px]:h-[40px] max-[1150px]:h-[38px] rounded-lg flex items-center">
                                <multiselect
                            ...
                            <div v-if="notFoundUsersByBranch" class="text-danger text-xs my-1">هیچ کاربری با این شعبه یافت نشد.</div>
                        ...
                        <div :class="{ 'has-error': $branchFormValidation.branchForm.registrationCode.$error }">
                            <label class="text-[16px] font-normal text-[#616161] max-[1150px]:text-[14px]" for="customeRegistrationCode">شماره ثبت:</label>
                            <div class="relative flex items-center">
                                <input
                            ...
                            <div class="flex flex-col">
                                ...
                                >
                                    <input
                                        ...
                                        accept=""
                                    />
                                    <div class="absolute left-4 !border-none w-[40px] h-[40px] flex justify-center items-center md:h-[70px] md:w-[70px]">
...
<script lang="ts" setup>
    ...
    const notFoundUsersByBranch = ref(false);
    ...
    let branchForm = ref<IbranchForm>({
        name: '',
        instance: '',
        managerId: '',
        economicCode: '',
    ...
    const edit_branch_data = async () => {
        ...
        return new Promise((resolve, reject) => {
            taxios(import.meta.env.VITE_MAIN_URL, 'multipart/form-data')
                ?.put('/branches/' + route.params.Id, formData)
                .then((res) => {
                    let branch = res.data.content;
                    let {
                        name,
                        instance,
                        managerId,
    ...
    const fetch_branch_data = () => {
        taxios()
            ?.get('/branches/' + route.params.Id)
            .then((res) => {
                ...
                branchForm.value.managerId = branch.manager.id.toString();
                _tempData = { ...branchForm.value };
            })
            .catch((error) => {
                if (error?.response?.data?.info?.[0]?.message) {
                    showAlert(error?.response?.data?.info?.[0]?.message?.fa || error?.response?.data?.info?.[0]?.message);
                }
            });
    };
    ...
    const update_selected_manager = (data) => {
        branchForm.value.managerId = data.id.toString();
    };

    const activateBranch = () => {
        taxios()
            ?.put(`/branches/changeStatus/${route.params.Id}/?status=${branchForm.value['isActive'] ? '0' : '1'}`)
            .then((res) => {
    ...
    onMounted(async () => {
        await fetch_branch_data();
        await fetch_managers();
        if (branchForm.value['isActive'] == false) {
            showAlert('این شعبه غیرفعال میباشد');
        }
    });
</script>
